...
export const PartnersCards = () => {
  const PartnersList = [
    { img: topstr, title: "TOPSTRETCHING — международная сеть студий растяжки и фитнеса, предлагающая 3 базовых направления: aerostretching, trx и школа шпагата. Заниматься можно и онлайн, а все локации находятся в шаговой доступности от метро. Скидка 30% всем студентам и преподавателям ЭМИТ.", href:"https://topstretching.com" },
    { img: "", title: "" },
    {
      img: pekarnya,
      title:
        "Если ты задумывался, где можно провести время после учебы, то компьютерный клуб Пекарня будет отличным выбором. В нём ты сможешь посетить зону приставок и лаунж, где можно перекусить и отдохнуть, а находится клуб совсем рядом!",
      href:"https://peka.club"
    },
    { img: "", title: "" },
    {
      img: strel,
      title:
        "Пневматический тир «Майор Ник» новый развивающийся проект в сфере развлечения и спорта. Здесь перед тобой будет представлен большой выбор оружия, от кольта до винтовки Мосина.",
        href:"https://tir-major-nik.ru/"
      },
    { img: "", title: "" },
    { img: mart, title: "MART — студия художественной вышивки, которая выполняет дизайны любой сложности: от логотипов до картин известных художников! Студия поможет воплотить любую твою задумку. Скидка 15% студентам ЭМИТ.", href:"https://martsew.ru"},
    { img: "", title: "" },
    { img: gonchar, title:"Гончар и художник — это творческая студия, предлагающая мастер-классы и индивидуальные занятия за гончарным кругом, по ручной лепке и живописи. Скидка 10% для студентов и сотрудников Академии.", href:"https://gonchar-hudozhnik.ru/"},
    { img: "", title: "" },
    { img: "", title: "" },
    { img: planum, title: "Стильные и качественные блокноты подойдут любителям планировать, рисовать или вести записи. Они точно помогут сделать твой день более продуктивным.", href:"https://planumbooks.com" },
    { img: "", title: "" },
    { img: candles, title: "YAM.CANDLES – бренд натуральных свечей и подсвечников из гипса ручной работы. Изделия из кокосового воска помогут создать уют в твоём доме, сделав вечера максимально комфортными и атмосферными. Каждый подсвечник с неповторимым узором будет идеальным дополнением в твой интерьер. А сами свечи будут приносить тебе тепло и уют!", href:"https://vk.com/yam.candles" },
    { img: "", title: "" },

    { img: neon, title: "Ребята занимаются разработкой и производством уникальных изделий из гибкого неона для дома и бизнеса. Ты можешь заказать как готовые решения, так и создать что-то эксклюзивное. А дизайнеры Simple Neon помогут визуализировать любую твою задумку. Сделай свой интерьер более ярким, интересным и атмосферным!", href:"https://vk.com/simpleneon" },
    { img: "", title: "" },

    { img: shoper, title: "Делаем шоперы из эко-материалов и наносим на них классные рисунки!", href:"https://taplink.cc/eco.shop.er" },
    { img: "", title: "" },
    { img: "", title: "" },
    { img: printer, title: "Печать моментальных фотографий может стать изюминкой любого мероприятия и запечатлеть миг не только в сердце, но и в твоём фотоальбоме.", href:"https://vk.com/away.php?to=https%3A%2F%2Femotionprinter.ru%2F&post=-106590225_6880&cc_key=" },
    { img: "", title: "" },
    { img: veco, title: "Мы занимаемся репродукцией и отчистки природы в повседневных вещах. Унас есть качественная продукция из эко-материалов", href:"https://vk.com/veco_app" },
    ...
    <div className="PartnerCard">
      ...
      <div className="partners">
        {PartnersList.map((item, index) => {
          return (
            <PartnerCard key={index} img={item.img} span={item.title} href={item.href} />
          );
        })}
      </div>
    </div>
...
export const PartnersRegistration = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_0x6w4rm', 'template_m9aowko', form.current, 'cMT7jbpxhcUcgCNXo')
      .then((result) => {
  ...
  return(
    ...
    <div className="PartnersLetter">
      <form action="#" className="partners-form" method="get" ref={form} onSubmit={sendEmail}>
        ...
        <button type='submit' className="PartnersButton" onClick={showMassage}>ПОЛУЧИТЬ ПРЕДЛОЖЕНИЕ</button>
      </form>
    </div>
